import leftPanelReducer, { initialState } from '../reducer'
import * as actions from '../actions'

describe('leftPanelReducer', () => {
  it('returns the initial state', () => {
    expect(leftPanelReducer(undefined, {} as any)).toEqual(initialState)
  })

  describe('REST', () => {
    it('FETCH_STATUS sets loading and clears error', () => {
      const state = { ...initialState, error: 'prev' }
      const result = leftPanelReducer(state, actions.fetchStatus())
  ...
  describe('SSE', () => {
    it('SSE_CONNECT sets streaming and resets log', () => {
      ...
      const result = leftPanelReducer({ ...initialState, streamLog: [event] }, actions.sseConnect())
