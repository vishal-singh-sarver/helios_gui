import centerWorkspaceReducer, { initialState } from '../reducer'
import * as actions from '../actions'

describe('centerWorkspaceReducer', () => {
  it('returns the initial state', () => {
    expect(centerWorkspaceReducer(undefined, {} as any)).toEqual(initialState)
  })

  describe('REST', () => {
    it('FETCH_STATUS sets loading and clears error', () => {
      const state = { ...initialState, error: 'prev' }
      const result = centerWorkspaceReducer(state, actions.fetchStatus())
    ...
    it('FETCH_STATUS_SUCCESS stores status and clears loading', () => {
      ...
      const result = centerWorkspaceReducer(
  ...
  describe('SSE', () => {
    ...
    it('SSE_EVENT appends event to streamLog', () => {
      ...
      const result = centerWorkspaceReducer(initialState, actions.sseEvent(event))
    ...
    it('SSE_EVENT does not mutate original state', () => {
      ...
      centerWorkspaceReducer(initialState, actions.sseEvent(event))
